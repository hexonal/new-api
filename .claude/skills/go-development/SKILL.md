...
name: go-development
description: |
  编写符合 Go 1.23+ 最佳实践的后端服务代码，使用 Gin/GORM 框架。
  适用于：创建/修改 .go 文件、编写 Handler/Service/Repository、
  处理 go.mod 配置、实现并发模式、设计三层架构。
  触发关键词：Go、Golang、Gin、GORM、后端、goroutine、并发。
  ⚠️ 编写 Go 代码时必须严格遵守此规范的强制规则，违反将导致代码审查不通过。
---
