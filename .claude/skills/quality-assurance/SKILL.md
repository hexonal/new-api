...
description: |
  启用代码质量保证能力，执行类型检查、代码规范验证、E2E 测试。
  适用于：运行 TypeScript 类型检查、执行 ESLint 规范验证、
  使用 Playwright MCP 进行 E2E 测试、生成质量报告、实施门禁规则。
  触发关键词：质量检查、QA、lint、typecheck、测试、E2E、门禁。
  互补规范：parallel-executor（并行任务执行）、frontend-development（前端开发）。
---
...
| 类型 | Skill | 说明 |
|------|-------|------|
| **互补** | [parallel-executor](../parallel-executor/SKILL.md) | 并行任务执行 |
| **互补** | [frontend-development](../frontend-development/SKILL.md) | 前端代码验证 |
| **互补** | [typescript-development](../typescript-development/SKILL.md) | TypeScript 代码验证 |
