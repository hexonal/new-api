...
func TestModelPriceHelperPerCall_FallsBackToBaseModelRatioForHailuoSKU(t *testing.T) {
	originalModelPrices := ratio_setting.ModelPrice2JSONString()
	originalGroupModelRatios := ratio_setting.GroupModelRatio2JSONString()
	defer func() {
		if err := ratio_setting.UpdateModelPriceByJSONString(originalModelPrices); err != nil {
	...
	if err := ratio_setting.UpdateGroupModelRatioByJSONString(`{"vip":{"MiniMax-Hailuo-2.3-Fast":0.8}}`); err != nil {
		t.Fatalf("update group model ratios: %v", err)
	}
	...
	if priceData.GroupRatioInfo.GroupRatio != 0.8 {
		t.Fatalf("group ratio = %v, want 0.8", priceData.GroupRatioInfo.GroupRatio)
	}
	if priceData.ModelPrice != 0.34 {
		t.Fatalf("model price = %v, want 0.34", priceData.ModelPrice)
	}
	if priceData.Quota != 136000 {
		t.Fatalf("quota = %d, want 136000", priceData.Quota)
	}
}

func TestModelPriceHelperPerCall_PrefersSKURatioOverBaseModelRatioForHailuo(t *testing.T) {
	originalModelPrices := ratio_setting.ModelPrice2JSONString()
	originalGroupModelRatios := ratio_setting.GroupModelRatio2JSONString()
	defer func() {
		if err := ratio_setting.UpdateModelPriceByJSONString(originalModelPrices); err != nil {
	...
	if err := ratio_setting.UpdateGroupModelRatioByJSONString(`{"vip":{"MiniMax-Hailuo-2.3-Fast-6s-1080p":0.5,"MiniMax-Hailuo-2.3-Fast":0.8}}`); err != nil {
		t.Fatalf("update group model ratios: %v", err)
	}
