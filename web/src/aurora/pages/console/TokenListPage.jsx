...
export default function TokenListPage() {
  ...
  }, [data, selectedTokenIds]);

  useEffect(() => {
    const existingIds = new Set((data.tokens || []).map((item) => item.id));
    setSelectedTokenIds((prev) => prev.filter((id) => existingIds.has(id)));
  }, [data.tokens]);
  ...
  const indeterminate =
    selectedTokenIds.length > 0 && selectedTokenIds.length < (data.tokens || []).length;
  ...
  const handleBatchDelete = () => {
    ...
    const confirmed = window.confirm(
      t('确认删除已选中的 {{count}} 个令牌吗？', { count: selectedTokenIds.length }),
    );
    if (!confirmed) {
      return;
    }
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex flex-col gap-3 rounded-xl border border-border bg-card/70 p-3'>
        ...
        <Table>
          ...
          <Tbody>
            {(data.tokens || []).map((token) => (
              <Tr key={token.id} className={token.status === 1 ? '' : 'bg-muted/40'}>
                <Td>
                  <input
                    type='checkbox'
