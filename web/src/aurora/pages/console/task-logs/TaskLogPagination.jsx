...
export default function TaskLogPagination({
  activePage,
  loading,
  logCount,
  onPageChange,
  ...
  const start = Math.min((activePage - 1) * pageSize + 1, logCount || 0);
  const end = Math.min(activePage * pageSize, logCount || 0);
  ...
  return (
    <section className='flex flex-col gap-4 rounded-xl border border-outline-variant bg-surface-container-lowest px-6 py-4 md:flex-row md:items-center md:justify-between'>
      <div className='text-sm text-on-surface-variant'>
        ...
        <span className='font-semibold text-on-surface'>{logCount || 0}</span>{' '}
        {t('tasks')}
      </div>
