  ...
  if (group === '') {
    return (
      <Tag
        ...
        color='white'
        shape='circle'
        className='group-tag group-tag-default'
      >
        {i18next.t('用户分组')}
      </Tag>
    );
...
function renderPriceSimpleCore({
  ...
  const { symbol, rate } = getCurrencyConfig();
  if (hasDirectModelPrice(modelPrice)) {
    if (hasDynamicPerCallRatios(otherRatios)) {
      ...
      if (dynamicPrice) {
        return buildDynamicPerCallFormula({
          ...
          rate,
          labels: {
            basePrice: i18next.t('基础单价'),
            groupRatio: ratioLabel,
            perCall: i18next.t('次'),
          },
        });
      }
      ...
      parts.push(
        i18next.t('缓存读取 {{price}}', {
          price: formatCompactDisplayPrice(
            normalizedModelRatio * 2.0 * cacheRatio,
          ),
        }),
      );
    }
    ...
    if (hasSplitCacheCreation && shouldShowCacheCreation5m) {
      parts.push(
        i18next.t('5m缓存创建 {{price}}', {
          price: formatCompactDisplayPrice(
            normalizedModelRatio * 2.0 * cacheCreationRatio5m,
          ),
        }),
      );
    }
    if (hasSplitCacheCreation && shouldShowCacheCreation1h) {
      parts.push(
        i18next.t('1h缓存创建 {{price}}', {
          price: formatCompactDisplayPrice(
            normalizedModelRatio * 2.0 * cacheCreationRatio1h,
          ),
        }),
      );
    }
    if (!hasSplitCacheCreation && shouldShowLegacyCacheCreation) {
      parts.push(
        i18next.t('缓存创建 {{price}}', {
          price: formatCompactDisplayPrice(
            normalizedModelRatio * 2.0 * cacheCreationRatio,
          ),
        }),
      );
    }
    ...
    if (image) {
      parts.push(
        i18next.t('图片输入 {{price}}', {
          price: formatCompactDisplayPrice(
            normalizedModelRatio * 2.0 * imageRatio,
          ),
        }),
      );
    }
...
export function renderModelPrice(
  ...
  const hasModelPrice =
    Number.isFinite(normalizedModelPrice) && normalizedModelPrice > 0;
  const normalizedModelRatio = Number(modelRatio);
  const hasModelRatio = Number.isFinite(normalizedModelRatio);
  ...
  if (
    !shouldUseRatioBillingProcess(hasModelPrice ? normalizedModelPrice : -1)
  ) {
    if (hasModelPrice) {
      if (hasDynamicPerCallRatios(otherRatios)) {
        const dynamicPrice = calculateDynamicPerCallPrice({
          modelPrice: normalizedModelPrice,
          groupRatio,
          otherRatios,
        });
        const parameterText = buildDynamicPerCallParameterText(otherRatios);
        if (dynamicPrice) {
          return (
            <>
              <article>
                <p>
                  {i18next.t('基础单价：{{symbol}}{{price}} / 次', {
                    symbol,
                    price: (dynamicPrice.basePrice * rate).toFixed(6),
                  })}
                </p>
                {parameterText && (
                  <p>
                    {i18next.t('计算参数')}：{parameterText}
                  </p>
                )}
                <p>
                  {buildDynamicPerCallFormula({
    ...
    const completionRatioPrice =
      normalizedModelRatio * 2.0 * safeCompletionRatio;
...
export function renderLogContent(
  ...
  if (
    isPriceDisplayMode(displayMode, hasModelPrice ? normalizedModelPrice : -1)
  ) {
    if (hasModelPrice) {
      if (hasDynamicPerCallRatios(otherRatios)) {
        const dynamicPrice = calculateDynamicPerCallPrice({
  ...
  if (hasModelPrice) {
    ...
    return i18next.t(
      '(模型价格 {{symbol}}{{price}} / 次) * {{ratioType}} {{ratio}}',
      {
        symbol: symbol,
        price: (normalizedModelPrice * rate).toFixed(6),
        ratioType: ratioLabel,
